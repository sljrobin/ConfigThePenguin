...
function xmonad_update()
{
  ...
  if [ ! -d "$DIR_XMONAD_OS" ]; then
    ...
    colorize "Updating xmonad [3/${STP_XMD_UPDATE}] Copying the folder 'data/'" $CLR_LBLUE "y"
    cp -r "$DIR_XMONAD_OS/data" "$DIR_XMONAD_REPO"
    colorize "Updating xmonad [4/${STP_XMD_UPDATE}] Copying the folder 'lib/'" $CLR_LBLUE "y"
    cp -r "$DIR_XMONAD_OS/lib/" "$DIR_XMONAD_REPO"
    colorize "Updating xmonad [5/${STP_XMD_UPDATE}] Removing the '*.hi' files in the folder 'lib/'" $CLR_LBLUE "y"
    rm "$DIR_XMONAD_REPO/lib"/*.hi
    colorize "Updating xmonad [6/${STP_XMD_UPDATE}] Removing the '*.o' files in the folder 'lib/'" $CLR_LBLUE "y"
